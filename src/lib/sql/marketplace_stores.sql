...
-- Policy ini mengizinkan pengguna yang sudah login untuk MEMBACA (SELECT) data.
CREATE POLICY "Allow authenticated read access" ON public.marketplace_stores
FOR SELECT USING (auth.role() = 'authenticated');

-- Policy ini mengizinkan pengguna yang sudah login untuk mengelola (INSERT, UPDATE, DELETE) data.
CREATE POLICY "Allow authenticated users to manage stores" ON public.marketplace_stores
FOR ALL USING (auth.role() = 'authenticated')
WITH CHECK (auth.role() = 'authenticated');

-- 5. Beri tahu Supabase tentang perubahan skema
