...
import { Loader2, ChevronLeft, ChevronRight, AlertCircle, PackageCheck, Download } from 'lucide-react';
...
export default function OutboundMonitoringPage() {
    ...
    const handleExport = () => {
        ...
        link.setAttribute("download", `outbound_monitoring_${format(new Date(), "yyyyMMdd")}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        toast({ title: "Success", description: "Outbound data has been exported." });
    };

    return (
        <MainLayout>
             <div className="w-full space-y-6">
                <h1 className="text-2xl font-bold">Outbound Monitoring</h1>
                {error && (
                    ...
                    <CardHeader>
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                            <div className="flex-1">
                                <CardTitle>Packed Order History</CardTitle>
                                <CardDescription>A log of all orders that have been packed and are ready for shipment.</CardDescription>
                            </div>
